...
export default {
  ...
  methods:{
    atualizarLista(){
      this.$http.get('https://localhost:50001/api/ListarTarefas')
        .then(res => res.json())
        .then(tarefas => this.tarefas = tarefas, err => console.log(err));
    },
    novaTarefa(){
      if(this.inputTarefa != ""){
        let endpoint = 'https://localhost:50001/api/NovaTarefa/'+this.inputTarefa
        this.realizaChamadaEExibeAlerta(endpoint);
      }
      this.atualizarLista();
    },
    deletaTarefa(id){
      let endpoint = 'https://localhost:50001/api/RemoverTarefa/'+id;
      this.realizaChamadaEExibeAlerta(endpoint);
      this.atualizarLista();
    },
    finalizarTarefa(id){
      let endpoint = 'https://localhost:50001/api/FinalizarTarefa/'+id;
      this.realizaChamadaEExibeAlerta(endpoint);
      this.atualizarLista();
    },
    finalizarTarefaSilenciosamente(id){
        this.$http.get('https://localhost:50001/api/FinalizarTarefa/'+id)
        .then(res => res.json())
    ...
    editarTarefa(id){
      ...
      let endpoint = 'https://localhost:50001/api/EditarTarefa/'+id+'/'+descricao;
      if(descricao!=null){
        this.realizaChamadaEExibeAlerta(endpoint);
      }
      this.atualizarLista();
